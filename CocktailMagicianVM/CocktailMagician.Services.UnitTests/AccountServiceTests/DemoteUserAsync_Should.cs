...
{
    [TestClass]
    public class DemoteUserAsync_Should
    {
        [TestMethod]
        public async Task DemotesUser()
        {
            var userId = 1;
            var type = "Cocktail Magician";
            var expected = "Bar Crawler";
            ...
            var options = TestUtilities.GetOptions(nameof(DemotesUser));
            ...
            using (var actContext = new CocktailDatabaseContext(options))
            {
                ...
                await sut.DemoteUserAsync(userId);
            }
